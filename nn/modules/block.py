...
class SeBlock(nn.Module):
    def __init__(self, in_channel, reduction=4, use_silu=False):
        super().__init__()
        ...
        if not use_silu:
            ...
            self.Excitation.add_module('SiLU', nn.SiLU())
    def forward(self, x):
        y = self.Squeeze(x)
...
class DepthSepConv(nn.Module):
    def __init__(self, inp, oup, dw_size, stride, use_se, use_DP=False, use_silu=False):
        ...
        if not use_DP:
            self.dw_sp = nn.Sequential(
                ...
                nn.Hardswish(),
                SeBlock(self.inp, reduction=16, use_silu=use_silu) if use_se else nn.Sequential(),
            ...
            self.dw_sp = nn.Sequential(
                ...
                nn.Hardswish(),
                SeBlock(self.inp, reduction=16, use_silu=use_silu) if use_se else nn.Sequential(),
                nn.Conv2d(self.inp, self.inp // 2, kernel_size=1, stride=1, padding=0, bias=False), S
                nn.BatchNorm2d(self.inp // 2),
                nn.Hardswish(),
                nn.Conv2d(self.inp // 2, self.oup, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.oup),
                nn.Hardswish()
            )
    def forward(self, x):
...
class RepDepthSepConv(nn.Module):
    def __init__(self, inp, oup, dw_size, stride, use_se, use_DP=False, use_silu=False):
        super(RepDepthSepConv, self).__init__()
        self.stride = stride
        self.inp = inp
        self.oup = oup
        self.dw_size = dw_size
        if not use_DP:
            self.dw_sp = nn.Sequential(
                RepConv(self.inp, self.inp, s=self.stride, g=self.inp, act=nn.Hardswish(), bn=True),
                SeBlock(self.inp, reduction=16, use_silu=use_silu) if use_se else nn.Sequential(),
                nn.Conv2d(self.inp, self.oup, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.oup),
                nn.Hardswish()
            ...
            self.dw_sp = nn.Sequential(
                RepConv(self.inp, self.inp, s=self.stride, g=self.inp, act=nn.Hardswish(), bn=True),
                SeBlock(self.inp, reduction=16, use_silu=use_silu) if use_se else nn.Sequential(),
                nn.Conv2d(self.inp, self.inp // 2, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.inp // 2),
                nn.Hardswish(),
                nn.Conv2d(self.inp // 2, self.oup, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.oup),
